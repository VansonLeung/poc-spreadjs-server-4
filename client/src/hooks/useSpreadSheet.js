...
export const useSpreadSheet = () => {
  ...
  useEffect(() => {
    ...
    window.addEventListener('spreadsheet-event', handleSpreadsheetEvent);

    return () => {
      window.removeEventListener('spreadsheet-event', handleSpreadsheetEvent);
    };
  }, []);
        ...
        try {
          ...
          events.forEach(eventType => {
            spread.bind(window.GC.Spread.Sheets.Events[eventType], (sender, args) => {
              ...
              window.dispatchEvent(new CustomEvent('spreadsheet-event', {
                detail: {
  ...
  return { containerRef, loading, debugInfo };
};